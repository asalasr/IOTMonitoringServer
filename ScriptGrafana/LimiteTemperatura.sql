...
min(v.valor) as min,
max(v.valor) as max,
(select d.min_value as Limite_diario_minimo FROM receiver_measurement as d where id=1),
(select d.max_value as Limite_diario_maximo FROM receiver_measurement as d where id=1)
FROM time_p t 
